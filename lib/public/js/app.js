...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location',
  function ($scope, $http, $location) {
      ...
      switch (btnName) {
        case 'download':
          if (FM.selection.length === 0) return true;
            ...
            if (!FM.IsAdmin) return true;
            else return false;
          }
        case 'delete':
        case 'move':
        case 'archive':
          return (FM.selection.length === 0) || !FM.IsAdmin;
        case 'rename':
          return (FM.selection.length !== 1) || !FM.IsAdmin;
        case 'upload_file':
        case 'create_folder':
          return false;
        default:
    ...
    FM.prebtnDisabled = function (btnName) {
      switch (btnName) {
        case 'download':
          if (FM.selection.length === 0) return true;
          else {
    ...
    FM.verification = function (Admincodes) {
      FM.IsAdmin = angular.equals(Admincodes, FM.Admincode);
      return FM.IsAdmin;
    };
    FM.Login = function (Admincodes) {
      var url = 'api' + FM.curFolderPath + encodeURI(Admincodes);
      httpRequest('POST', url, {type: 'CREATE_FOLDER'}, null);
    };
